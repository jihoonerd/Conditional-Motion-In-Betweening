...
from rmi.model.preprocess import replace_noise, vectorize_pose, replace_inpainting_range,  vectorize_representation, interpolate_input_repr
...
def test(opt, device):
    ...
    pose_vectorized_gt = global_pose_vec_gt.permute(1,0,2)
    ...
    for i in range(len(test_idx)):
        ...
        for t in range(horizon):
            ...
            plot_pose_with_stop(start_pose, pos_pred[test_idx[i],t].reshape(lafan_dataset.num_joints, 3).detach().numpy(), target_pose, stopover_pose, t, skeleton_mocap, save_dir=pred_img_path, prefix='pred')
