...
from rmi.vis.pose import plot_pose, plot_pose_compare
...
def test(opt, device):
    ...
    lstm.eval()
    decoder.eval()
    cond_cont_code = list(range(0,15))
            ...
            with torch.no_grad():
                ...
                for cont_value in c:            
                    ...
                    for t in range(training_frames):
                        ...
                        pred_pose.append(in_between_true)
                if opt.plot :
                    for t in range(training_frames):
                        plot_pose_compare(start_pose, pred_pose[t], pred_pose[t], target_pose, t, skeleton, save_dir=save_dir, pred=True)
                ...
                if opt.plot:
                    # if i_batch < 49:
                    gif_path = os.path.join(opt.save_dir, 'img'+str(cont_value)+'__'+'cond'+str(conditioning_cont_code)+'_%02d.gif' % i_batch)
