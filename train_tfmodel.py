...
from rmi.model.network import (DInfoGAN, InfoGANCRH, MotionDiscriminator,
                               QInfoGAN, TransformerModel)
...
def train(opt, device):
    ...
    flip_bvh(opt.data_path) # Flip, Load and preprocess data. It utilizes LAFAN1 utilities
    ...
    generator_optim_scheduler = torch.optim.lr_scheduler.StepLR(generator_optim, step_size=200, gamma=0.75)
    ...
    discriminator_optim = Adam(params=list(infogan_discriminator.parameters()) +
                                      ...
                                      betas=(opt.optim_beta1, opt.optim_beta2))
    discriminator_optim_scheduler = torch.optim.lr_scheduler.StepLR(discriminator_optim, step_size=200, gamma=0.1)

    cr_optim = Adam(params=list(transformer_encoder.parameters()) + 
                           list(infogan_crh.parameters()), 
                           lr=opt.cr_learning_rate, 
                           betas=(opt.optim_beta1, opt.optim_beta2))
    cr_optim_scheduler = torch.optim.lr_scheduler.StepLR(cr_optim, step_size=200, gamma=0.75)

    scaler = amp.GradScaler(enabled=cuda)

    for epoch in range(1, epochs + 1):

        pbar = tqdm(lafan_data_loader, desc="Batch")
        ...
        generator_optim_scheduler.step()
        discriminator_optim_scheduler.step()
        cr_optim_scheduler.step()
...
def parse_opt():
    ...
    parser.add_argument('--cr_learning_rate', type=float, default=0.0001, help='crh_infogan learning rate')
    ...
    parser.add_argument('--loss_generator_weight', type=float, default=0.5, help='loss_gan_weight')
    parser.add_argument('--loss_code_weight', type=float, default=0.5, help='loss_code_weight')
